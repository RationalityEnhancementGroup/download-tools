...
def explode_questionnaire_df(questionnaire_df, additional_columns=None):
    """
    ...
    for additional_column, default_val in additional_columns.items():
        questionnaire_df[additional_column] = questionnaire_df.apply(
            lambda row: process_additional_column_to_explode(
                row, additional_column, default_val  # noqa: B023
            ),
            axis=1,
        )
...
def get_quiz_passer_ids(
    survey_data, max_attempts=4, passing_score=4, identifying_columns=None
):
    """
...
def score_mouselab_questionnaires(
    ...
    mouselab_questionnaires["responses"] = mouselab_questionnaires["responses"].apply(
        lambda entry: eval(entry)
        if (isinstance(entry, str) and not pd.isnull(entry))
        else entry
    )
    mouselab_questionnaires[accuracy_string] = mouselab_questionnaires[
        accuracy_string
    ].apply(
        lambda entry: eval(entry)
        if (isinstance(entry, str) and not pd.isnull(entry))
...
def score_generic_questionnaires(
    questionnaires,
    solutions_dict,
    group_identifier,
    accuracy_string="correct",
    ...
    additional_columns = {}
    for questionnaire_col in [accuracy_string, "questions"] + [
        "reverse_coded",
        "open_ended",
    ]:
    ...
    # score dataframe
    exploded_questionnaires["score"] = exploded_questionnaires.apply(
        lambda row: score_row(
            row,
            solutions_dict[row["run"]],
            ...
            default_open_ended=default_open_ended[row["name"]]
            if row["name"] in default_open_ended
            else None,
        ),
        axis=1,
    )
