...
def get_participant_bonus(
    ...
    """
    Pull out all bonus fields from trial, question and general data.
    ...
    bonus_df = (
        ...
        .groupby(["workerid", "assignmentid"])["score"]
        .sum()
...
def save_participant_files(
    ...
    if save_path is None:
        ...
        data_path = parent_dir / "data" / "human" / f"{exp_name}"
    else:
        data_path = Path(f"{save_path}") / f"{exp_name}"
    ...
    general_info.to_csv(data_path.joinpath("general_info.csv"), index=False)
    question_data.to_csv(data_path.joinpath("question_data.csv"), index=False)
    event_data.to_csv(data_path.joinpath("event_data.csv"), index=False)
    ...
    if "score" in trial_data:
        ...
        bonus_df.drop_duplicates().to_csv(
            data_path.joinpath("bonuses.csv"), index=False
        )
    ...
    for trial_type in np.unique(trial_data["trial_type"]):
        trial_data[trial_data["trial_type"] == trial_type].to_csv(
            data_path.joinpath(f"{trial_type}.csv"), index=False
        )
